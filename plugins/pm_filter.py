...
from database.ia_filterdb import (Media, get_bad_files, get_file_details,
                                  get_search_results)
from database.users_chats_db import db
from info import (ADMINS, AUTH_CHANNEL, AUTH_GROUPS, AUTH_USERS, CHNL_LNK,
                  CUSTOM_FILE_CAPTION, GRP_LNK, IMDB, IMDB_TEMPLATE, NO_RESULTS_MSG,
                  LOG_CHANNEL, MAX_B_TN, MSG_ALRT, NOR_IMG, P_TTI_SHOW_OFF,
                  PICS, SINGLE_BUTTON, SPELL_CHECK_REPLY, SPELL_IMG, FILE_CHANNEL_LINK, FILE_CHANNEL, DELETE_TIME,
                  SUPPORT_CHAT_ID)
from Script import script
from utils import (get_poster, get_settings, get_size, is_subscribed,
                   save_group_settings, search_gagala, temp)
...
async def pm_text(bot, message):
    ...
    await message.reply_text(f"<b>𝖧𝖾𝗒 {query.from_user.first_name} Yᴏᴜʀ ᴍᴇssᴀɢᴇ ʜᴀs ʙᴇᴇɴ sᴇɴᴛ ᴛᴏ ᴍʏ ᴍᴏᴅᴇʀᴀᴛᴏʀs !</b>")
    ...
    try:
        ...
        if settings['max_btn']:
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("📜 𝖯𝖺𝗀𝖾", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("📜 𝖯𝖺𝗀𝖾", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("⏪ BACK", callback_data=f"next_{req}_{key}_{off_set}"),
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if (st.status == enums.ChatMemberStatus.OWNER) or (str(userid) in ADMINS):
                ...
                await query.answer(f"Hey, {query.from_user.first_name} Tʜᴀᴛ's ɴᴏᴛ ғᴏʀ ʏᴏᴜ!!", show_alert=True)
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                if clicked == typed:
                    ...
                    await query.answer(f"𝖧𝖾𝗒 {query.from_user.first_name}, Search Your Own File, Don't Click Others Results 😬", show_alert=True)
            elif settings['botpm']:
                if clicked == typed:
                    await query.answer(url=f"https://t.me/{temp.U_NAME}?start={ident}_{file_id}")
                    return
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer(f"Hᴇʏ {query.from_user.first_name} I Like Your Smartness, But Don't Be Oversmart Okay", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        if int(query.from_user.id) == int(from_user):
            ...
            await query.answer(f"Hᴇʏ {user.first_name}, Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        try:
            ...
            if settings['max_btn']:
                btn.append(
                    [InlineKeyboardButton("📜 𝖯𝖺𝗀𝖾", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="NEXT ⏩",callback_data=f"next_{req}_{key}_{offset}")]
                )
            else:
                btn.append(
                    [InlineKeyboardButton("📜 𝖯𝖺𝗀𝖾", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="NEXT ⏩",callback_data=f"next_{req}_{key}_{offset}")]
                )
        except KeyError:
